...
const CreditCardStatementCard: React.FC<CreditCardStatementCardProps> = ({ 
    ...
    const StatementBlock: React.FC<{ title: string; data: StatementInfo; isHighlight?: boolean }> = ({ title, data, isHighlight }) => {
        ...
        return (
            <div className={`p-2 rounded-xl border flex flex-col justify-between ${isHighlight ? 'bg-blue-50 dark:bg-blue-900/10 border-blue-100 dark:border-blue-800/30' : 'bg-gray-50 dark:bg-white/5 border-transparent'}`}>
                <div className="flex justify-between items-center mb-0.5">
                    <h4 className={`text-[10px] font-bold tracking-tight uppercase ${isHighlight ? 'text-blue-700 dark:text-blue-300' : 'text-light-text-secondary dark:text-dark-text-secondary'}`}>{title}</h4>
                </div>
                <div className="space-y-0.5">
                    <div className="flex justify-between items-center text-[11px]">
                        <span className="text-light-text-secondary dark:text-dark-text-secondary text-[10px]">Balance</span>
                        <span className="text-xs sm:text-sm font-bold text-light-text dark:text-dark-text">{formatCurrency(data.balance, currency)}</span>
                    </div>
                    <div className="flex justify-between items-center text-[11px]">
                        <span className="text-light-text-secondary dark:text-dark-text-secondary text-[10px]">Due Date</span>
                        <span className={`text-[10px] font-semibold ${title.includes("Current") ? 'text-light-text dark:text-dark-text' : 'text-light-text-secondary dark:text-dark-text-secondary'}`}>{data.dueDate}</span>
                    </div>
                </div>
                {title.includes("Current") && hasPreviousStatement && (
                     <div className="mt-1 pt-1 border-t border-black/5 dark:border-white/5 flex justify-between items-center text-[9px]">
                        <div className="flex items-center gap-1">
                            ...
                            {isPaid && (
                                <span className="flex items-center gap-0.5 font-semibold bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-1 py-0.2 rounded scale-90 origin-left">
                                    <span className="material-symbols-outlined text-[9px]">check</span> Paid
                                </span>
                            )}
                        </div>
                        <span className="font-mono text-[9px]">{formatCurrency(Math.abs(previousStatementBalance), currency)}</span>
                     </div>
                )}
            </div>
        );
    ...
    const content = (
        <div ref={containerRef} className={`flex ${isWide ? 'flex-row items-center' : 'flex-col'} gap-2.5 h-full`}>
            {/* Left/Top: Card Info */}
            <div className={`flex flex-col justify-center ${isWide ? 'w-1/4 border-r pr-2.5' : 'border-b pb-1.5'} border-black/5 dark:border-white/5`}>
                <div className="flex items-center gap-2 mb-0.5">
                    <div className="w-5 h-5 rounded-md bg-primary-100 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400 flex items-center justify-center shrink-0">
                        <span className="material-symbols-outlined text-[12px]">credit_card</span>
                    </div>
                    <h3 className="font-semibold text-xs text-light-text dark:text-dark-text truncate">{accountName}</h3>
                </div>
            ...
            <div className={`flex-1 ${isWide ? 'grid grid-cols-2 gap-2' : 'space-y-1.5'} w-full`}>
