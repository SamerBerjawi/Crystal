            ...
            {step === 'backup' && (
                <div className="space-y-4">
                    <div className="text-center space-y-2">
                        ...
                        <h3 className="text-xl font-bold text-light-text dark:text-dark-text">2FA Enabled!</h3>
                    ...
                    <div className="grid grid-cols-2 gap-2 bg-gray-100 dark:bg-black/30 p-4 rounded-lg font-mono text-sm border border-black/5 dark:border-white/10">
                        {backupCodes.map((code, idx) => (
                            <div key={idx} className="text-center tracking-widest text-light-text dark:text-dark-text">{code}</div>
                        ))}
                    </div>
                    
                    <button 
                        onClick={handleCopyCodes} 
                        className="w-full flex items-center justify-center gap-2 text-sm font-semibold text-primary-600 dark:text-primary-400 hover:underline"
                    >
                        <span className="material-symbols-outlined text-lg">{hasCopied ? 'check' : 'content_copy'}</span>
                        {hasCopied ? 'Copied!' : 'Copy to Clipboard'}
                    </button>
                    
                    <button onClick={handleFinish} className={`${BTN_PRIMARY_STYLE} w-full mt-4`}>
