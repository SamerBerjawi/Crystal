    ...
    <>
      {categories.map(cat => (
        <React.Fragment key={cat.id}>
          <option className="bg-white dark:bg-gray-900 text-black dark:text-white py-1" value={cat.name}>
            {indent}{cat.name}
          </option>
...
const CategoryOptions: React.FC<{ categories: Category[] }> = ({ categories }) => (
  <>
    {categories.map(parentCat => (
      <optgroup
        className="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 font-bold text-xs tracking-wider h-10"
        key={parentCat.id}
        label={parentCat.name}
      >
        <option className="bg-white dark:bg-gray-900 text-black dark:text-white font-medium py-2" value={parentCat.name}>
          {parentCat.name}
        </option>
...
}> = ({ field, label, isChecked, onToggle, children }) => (
  <div
    className={`flex flex-col gap-3 p-4 rounded-2xl border transition-all duration-200 ${
      isChecked
        ? 'bg-primary-50/50 dark:bg-primary-900/10 border-primary-300 dark:border-primary-800/40 shadow-sm'
    ...
    <div className="flex items-center gap-3">
      ...
      <label
        ...
        className={`text-xs font-bold uppercase tracking-wider cursor-pointer select-none transition-colors ${
          isChecked ? 'text-primary-600 dark:text-primary-400' : 'text-light-text dark:text-dark-text'
        }`}
      >
        {label}
      </label>
...
interface BulkEditTransactionsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (updatedTransactions: Transaction[], idsToDelete?: string[]) => void;
...
const BulkEditTransactionsModal: React.FC<BulkEditTransactionsModalProps> = ({
  ...
  const handleToggle = (field: string) => {
    setFieldsToUpdate(prev => ({
      ...prev,
      [field as keyof typeof fieldsToUpdate]: !prev[field as keyof typeof fieldsToUpdate],
    }));
  };

  const handleChange = (
    field: keyof Omit<typeof updatedValues, 'tagIds' | 'locationString' | 'locationData'>,
    value: string
  ) => {
  ...
  const isCategoryInvalid = fieldsToUpdate.category && !updatedValues.category.trim();
  const isAccountInvalid = fieldsToUpdate.accountId && !updatedValues.accountId;
  const isFormInvalid = activeFieldsCount === 0 || isCategoryInvalid || isAccountInvalid;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const categoryDetails = fieldsToUpdate.category ? findCategory(updatedValues.category, allCategories) : undefined;
    const newType = categoryDetails?.classification;

    transactionsToEdit.forEach(tx => {
      ...
      if (fieldsToUpdate.category && updatedValues.category) {
        ...
        if (tx.transferId) {
          updatedTx.transferId = undefined;
          const counterpart = allTransactions.find(t => t.transferId === tx.transferId && t.id !== tx.id);
          if (counterpart) {
            idsToDelete.push(counterpart.id);
          }
        }
      } else if (tx.transferId) {
        // If not re-categorized, keep transfer counterpart synchronized for date/merchant/description/tags/location
        const counterpart = allTransactions.find(t => t.transferId === tx.transferId && t.id !== tx.id);
        if (counterpart && !updatedTransactionsMap.has(counterpart.id)) {
          const updatedCounterpart: Transaction = { ...counterpart };
          if (fieldsToUpdate.date) updatedCounterpart.date = updatedValues.date;
          if (fieldsToUpdate.description) updatedCounterpart.description = updatedValues.description;
          if (fieldsToUpdate.merchant) updatedCounterpart.merchant = updatedValues.merchant;
          if (fieldsToUpdate.tags) updatedCounterpart.tagIds = updatedValues.tagIds;
          if (fieldsToUpdate.location) {
            const hasLoc = Boolean(updatedValues.locationString?.trim());
            updatedCounterpart.address = hasLoc ? (updatedValues.locationData?.address || updatedValues.locationString.trim()) : undefined;
            updatedCounterpart.placeName = hasLoc ? updatedValues.locationData?.placeName : undefined;
            updatedCounterpart.street = hasLoc ? updatedValues.locationData?.street : undefined;
            updatedCounterpart.city = hasLoc ? (updatedValues.locationData?.city || updatedValues.locationString.trim()) : undefined;
            updatedCounterpart.postalCode = hasLoc ? updatedValues.locationData?.postalCode : undefined;
            updatedCounterpart.state = hasLoc ? updatedValues.locationData?.state : undefined;
            updatedCounterpart.country = hasLoc ? updatedValues.locationData?.country : undefined;
            updatedCounterpart.latitude = hasLoc ? updatedValues.locationData?.lat : undefined;
            updatedCounterpart.longitude = hasLoc ? updatedValues.locationData?.lon : undefined;
            updatedCounterpart.locationLabel = hasLoc ? updatedValues.locationData?.locationLabel : undefined;
          }
          updatedTransactionsMap.set(updatedCounterpart.id, updatedCounterpart);
        }
      }

      updatedTransactionsMap.set(updatedTx.id, updatedTx);
    });

    onSave(Array.from(updatedTransactionsMap.values()), idsToDelete);
    handleClose();
  };
          ...
          <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
              <div className="p-4 bg-blue-500/10 text-blue-800 dark:text-blue-200 rounded-2xl flex items-start gap-3 text-xs border border-blue-500/20">
                ...
                <div>
                  ...
                  <p className="opacity-80">
                    Only checked fields below will be updated. Unchecked fields will remain untouched.
                  </p>
                </div>
              </div>
              ...
              <CheckboxField field="date" label="Adjust Date" isChecked={fieldsToUpdate.date} onToggle={handleToggle}>
                <div className="relative group">
                  <Icon
                    name="calendar_today"
                    className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-base group-focus-within:text-primary-500 pointer-events-none"
                  />
                  <input
                    type="date"
              ...
              <CheckboxField
                ...
                isChecked={fieldsToUpdate.accountId}
                onToggle={handleToggle}
              >
                <div className={SELECT_WRAPPER_STYLE}>
                  <select
                    value={updatedValues.accountId}
                    onChange={e => handleChange('accountId', e.target.value)}
                    className={`${SELECT_STYLE} h-11 pl-4 cursor-pointer font-bold`}
                    required={fieldsToUpdate.accountId}
                  >
                    {accounts.map(acc => (
                      <option className="bg-white dark:bg-gray-900 text-black dark:text-white" key={acc.id} value={acc.id}>
                        {acc.name}
                      </option>
                    ))}
                  </select>
              ...
              <CheckboxField
                field="description"
                label="Update Memo / Description"
                isChecked={fieldsToUpdate.description}
                onToggle={handleToggle}
              >
                <div className="relative group">
                  <Icon
              ...
              <CheckboxField
                ...
                isChecked={fieldsToUpdate.merchant}
                onToggle={handleToggle}
              >
                <div className="relative group">
                  <Icon
                  ...
                  <input
                    type="text"
                    value={updatedValues.merchant}
                    onChange={e => handleChange('merchant', e.target.value)}
                    className={`${INPUT_BASE_STYLE} pl-10 h-11`}
                    placeholder="New merchant or payee..."
                  />
                </div>
              </CheckboxField>
              ...
              <CheckboxField
                field="category"
                label="Re-categorize"
                isChecked={fieldsToUpdate.category}
                onToggle={handleToggle}
              >
                <div className={SELECT_WRAPPER_STYLE}>
                  <select
                    ...
                    className={`${SELECT_STYLE} h-11 pl-4 cursor-pointer font-bold`}
                    required={fieldsToUpdate.category}
                  >
                    <option className="bg-white dark:bg-gray-900 text-black dark:text-white" value="">
                      Select a category
                    </option>
                    <optgroup
                      className="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 font-semibold text-xs uppercase tracking-wider h-10"
                      label="Outgoing"
                    />
                ...
                {fieldsToUpdate.category && isCategoryInvalid && (
                  <p className="text-2xs text-rose-500 font-semibold mt-1 pl-1">Please select a destination category.</p>
                )}
                <div className="flex gap-2 items-start mt-3 p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-xs text-amber-800 dark:text-amber-300 font-semibold">
                  ...
                  <p>Categorizing updates the classification (income/expense) and normalizes transaction signs accordingly.</p>
                </div>
              </CheckboxField>
                  ...
                  >
                    {selectedTags.length > 0 ? (
                      selectedTags.map(tag => (
                        <span
                          key={tag.id}
                          className="flex items-center gap-1 text-xs px-2.5 py-0.5 rounded-full border border-black/5 dark:border-white/10 font-semibold"
                        ...
                        tags.map(tag => (
                          ...
                          >
                            ...
                            <div className="flex-1 flex justify-between items-center">
                              <span className="text-xs font-bold text-light-text dark:text-dark-text group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
                                {tag.name}
                              </span>
              ...
              <CheckboxField
                ...
                isChecked={fieldsToUpdate.location}
                onToggle={handleToggle}
              >
                <div>
                  <AddressAutocomplete
            ...
            <div className="p-6 border-t border-black/5 dark:border-white/5 bg-light-card/80 dark:bg-dark-card/80 backdrop-blur-md flex items-center justify-between gap-3">
              ...
              <button
                ...
                className={`${BTN_PRIMARY_STYLE} h-12 px-8 text-xs font-bold uppercase tracking-wider flex items-center gap-2 shadow-lg shadow-primary-500/20 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed`}
              >
                <span>
                  {activeFieldsCount === 0
                    ? 'Select Fields to Update'
                    : `Update ${activeFieldsCount} Field${activeFieldsCount !== 1 ? 's' : ''} on ${
                        transactionsToEdit.length
                      } Record${transactionsToEdit.length !== 1 ? 's' : ''}`}
                </span>
