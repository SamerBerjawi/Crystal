...
import { formatCurrency, calculateTrendLine } from '../utils';
  ...
  const StatBlock = ({
    ...
    const dataWithTrend = useMemo(() => {
      ...
      return data.map((d, i) => {
        ...
        itemDate.setDate(now.getDate() - (data.length - 1 - i));
        return {
          ...d,
          date: itemDate,
          trend: trendVals[i],
      ...
      <div className="flex flex-col h-full justify-between">
        ...
        <div className="h-10 w-full mt-2 opacity-60">
          {dataWithTrend.length > 0 && (
            ...
            >
              <Line dataKey="value" stroke={strokeColor} strokeWidth={2} fadeEdges />
              <Line dataKey="trend" stroke="#6366f1" strokeWidth={1.5} strokeDasharray="3,3" />
            </LineChart>
          )}
        </div>
      </div>
          ...
          <div>
            ...
            <h3
              className={`text-3xl font-bold tracking-tight ${
                isPositiveNet ? 'text-light-text dark:text-dark-text' : 'text-red-500'
              }`}
            >
