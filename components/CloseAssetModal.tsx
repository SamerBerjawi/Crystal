      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className="block text-xs font-semibold uppercase tracking-wider text-light-text-secondary dark:text-dark-text-secondary mb-2">Closure Type</label>
            <div className="flex bg-gray-100 dark:bg-white/5 p-1 rounded-xl gap-1 overflow-x-auto">
              {([
                ...
                <button
                  ...
                  className={`flex-1 py-2 px-3 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all whitespace-nowrap ${closureType === item.id ? 'bg-white dark:bg-dark-card shadow-sm text-primary-600' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}
                >
                  {item.label}
                </button>
              ))}
