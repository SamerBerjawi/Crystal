            ...
            <form onSubmit={handleSubmit} className="space-y-8 animate-fade-in-up">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="bg-white dark:bg-dark-card p-6 rounded-[2rem] border border-black/5 dark:border-white/5 shadow-sm text-center relative overflow-hidden group">
                        ...
                        <p className="text-xs font-semibold uppercase tracking-wider text-light-text-secondary dark:text-dark-text-secondary opacity-60 mb-2">Current Benchmark</p>
                        <p className="text-2xl font-black tabular-nums tracking-tight text-light-text dark:text-dark-text group-hover:text-primary-500 transition-colors">
                    ...
                    <div className="bg-white dark:bg-dark-card p-6 rounded-[2rem] border border-black/5 dark:border-white/5 shadow-sm text-center relative overflow-hidden group">
                        ...
                        <p className="text-xs font-semibold uppercase tracking-wider text-light-text-secondary dark:text-dark-text-secondary opacity-60 mb-2">Projected Appreciation</p>
                        <p className={`text-2xl font-black tabular-nums tracking-tight ${(parseFloat(newValue) - account.balance) >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                ...
                <div className="bg-gray-50/50 dark:bg-white/[0.02] rounded-[2.5rem] border border-black/5 dark:border-white/5 p-8 space-y-8">
                    <div className="space-y-6">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-xs font-semibold uppercase tracking-wider text-light-text-secondary dark:text-dark-text-secondary">Valuation Specifications</h3>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
                            <div className="md:col-span-3">
                                <label htmlFor="new-value" className="block text-xs font-semibold uppercase tracking-wider text-light-text-secondary dark:text-dark-text-secondary mb-3 ml-1">Market Assessment</label>
                                <div className="relative group">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white dark:bg-dark-card border border-black/5 dark:border-white/5 rounded-2xl pl-12 pr-6 h-14 text-lg font-bold outline-none focus:ring-4 focus:ring-primary-500/10 transition-all"
                                        required
                                        autoFocus
                                    />
                                </div>
                                <p className="mt-3 text-xs text-light-text-secondary/60 dark:text-dark-text-secondary/40 italic ml-1 leading-relaxed">The equity statement will be adjusted to reflect this updated market position.</p>
                            </div>
                            
                            <div className="md:col-span-2">
                                <label htmlFor="valuation-date" className="block text-xs font-semibold uppercase tracking-wider text-light-text-secondary dark:text-dark-text-secondary mb-3 ml-1">Effective Date</label>
                                <input
                                    id="valuation-date"
                                    type="date"
                                    value={date}
                                    onChange={(e) => setDate(e.target.value)}
                                    className="w-full bg-white dark:bg-dark-card border border-black/5 dark:border-white/5 rounded-2xl px-6 h-14 text-sm font-semibold outline-none focus:ring-4 focus:ring-primary-500/10 transition-all"
                                    required
                                />
                            </div>
                        </div>
                ...
                <div className="flex justify-end gap-4 p-2">
                    <button type="button" onClick={onClose} className="h-14 px-8 rounded-2xl text-xs font-semibold uppercase tracking-wider text-light-text-secondary dark:text-dark-text-secondary hover:bg-black/5 dark:hover:bg-white/5 transition-all">Cancel</button>
                    <button type="submit" className={`${BTN_PRIMARY_STYLE} h-14 px-10 rounded-2xl text-xs font-semibold uppercase tracking-wider shadow-lg shadow-primary-500/20`}>Confirm New Valuation</button>
                </div>
            </form>
            ...
            {account.priceHistory && account.priceHistory.length > 0 && (
                <div className="mt-12 pt-12 border-t border-black/5 dark:border-white/5">
                    <div className="flex items-center gap-3 mb-8 ml-2">
                        ...
                        <h3 className="text-xs font-semibold uppercase tracking-wider text-light-text-secondary dark:text-dark-text-secondary">Valuation History Ledger</h3>
                    </div>
                        ...
                        {[...account.priceHistory].reverse().map((entry, idx) => (
                            <div key={idx} className="group/entry bg-white dark:bg-dark-card p-5 rounded-[1.5rem] border border-black/5 dark:border-white/5 shadow-sm hover:shadow-md hover:border-primary-500/20 transition-all flex justify-between items-center relative overflow-hidden">
                                ...
                                <div className="space-y-1">
                                    <p className="text-base font-black text-light-text dark:text-dark-text tracking-tight tabular-nums">{formatCurrency(entry.price, account.currency)}</p>
                                    <p className="text-xs font-medium text-light-text-secondary/60 dark:text-dark-text-secondary/60">{parseLocalDate(entry.date).toLocaleDateString(undefined, { dateStyle: 'medium' })}</p>
                                </div>
