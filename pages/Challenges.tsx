...
const Challenges: React.FC<ChallengesProps> = ({ userStats, accounts, transactions, predictions, savePrediction, deletePrediction, saveUserStats, investmentTransactions, warrants, assetPrices }) => {
  ...
  const badges = useMemo(() => {
      ...
      return [
        ...
        { id: 'prophet', title: 'Prophet', description: 'Win 5 predictions.', icon: 'psychic', color: 'violet', unlocked: (userStats.predictionWins || 0) >= 5, progress: ((userStats.predictionWins || 0) / 5) * 100 },
      ];
          ...
          {activeSection === 'mastery' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-fade-in-up">
                  {categoryMastery.map((cat, idx) => (
                      <CategoryMasteryCard key={idx} categoryName={cat.categoryName} spent={cat.spent} budget={cat.amount} level={cat.level} title={cat.title} icon={cat.icon} categoryColor={cat.categoryColor} />
                  ))}
