...
try {
    const res = (0, urlValidator_1.isAllowedTargetUrl)('https://api.twelvedata.com/price?symbol=AAPL');
...
try {
    const res = (0, urlValidator_1.isAllowedTargetUrl)('https://generativelanguage.googleapis.com/v1beta/models');
...
try {
    const res = (0, urlValidator_1.isAllowedTargetUrl)('http://169.254.169.254/latest/meta-data/');
...
try {
    const res = (0, urlValidator_1.isAllowedTargetUrl)('http://127.0.0.1:3000/api/admin');
...
try {
    const res = (0, urlValidator_1.isAllowedTargetUrl)('http://10.0.0.1/secret');
...
try {
    const res = (0, urlValidator_1.isAllowedTargetUrl)('https://malicious-external-site.com/steal-data');
...
try {
    ...
    const extRes = (0, urlValidator_1.isAllowedTargetUrl)('https://custom-stock-site.com/price');
    assert(extRes.allowed === true, 'Wildcard allowlist permits external stock site');
    const metadataRes = (0, urlValidator_1.isAllowedTargetUrl)('http://169.254.169.254/latest');
