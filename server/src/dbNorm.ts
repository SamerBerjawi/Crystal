...
import { JsonPatchOperation } from './schemas';
 ...
 */
export async function fetchFinancialDataFromRelational(userId: number): Promise<Record<string, any>> {
    const profileRes = await db.query(`SELECT last_updated_at, data FROM user_financial_profiles WHERE user_id = $1`, [userId]);
    ...
    const txRes = await db.query(`SELECT * FROM transactions WHERE user_id = $1 ORDER BY date DESC`, [userId]);
    ...
    const goalRes = await db.query(`SELECT * FROM financial_goals WHERE user_id = $1`, [userId]);
    ...
    const recRes = await db.query(`SELECT * FROM recurring_transactions WHERE user_id = $1`, [userId]);
    ...
    const taskRes = await db.query(`SELECT * FROM tasks WHERE user_id = $1`, [userId]);
 ...
 */
export async function syncFinancialDataToRelational(
    userId: number,
    ...
    try {
        ...
        if (Array.isArray(nextData.accounts)) {
            ...
            for (const acc of nextData.accounts) {
                ...
                await client.query(
                    `INSERT INTO accounts (id, user_id, name, type, balance, currency, data)
                     VALUES ($1, $2, $3, $4, $5, $6, $7)`,
                    [id, userId, name || 'Account', type || 'Checking', balance || 0, currency || 'EUR', JSON.stringify(rest)]
                );
            }
        }
        ...
        if (Array.isArray(nextData.transactions)) {
            ...
            for (const tx of nextData.transactions) {
                ...
                await client.query(
                    `INSERT INTO transactions (id, user_id, account_id, amount, date, description, category, type, currency, data)
        ...
        if (Array.isArray(nextData.budgets)) {
            ...
            for (const b of nextData.budgets) {
                ...
                await client.query(
                    `INSERT INTO budgets (id, user_id, name, amount, category, period, data)
                     VALUES ($1, $2, $3, $4, $5, $6, $7)`,
                    [id, userId, name || 'Budget', amount || 0, category || 'All', period || 'Monthly', JSON.stringify(rest)]
                );
            }
        }
        ...
        if (Array.isArray(nextData.financialGoals)) {
            ...
            for (const g of nextData.financialGoals) {
                ...
                await client.query(
                    `INSERT INTO financial_goals (id, user_id, name, target_amount, current_amount, currency, data)
                     VALUES ($1, $2, $3, $4, $5, $6, $7)`,
                    [id, userId, name || 'Goal', target, currentAmount || 0, currency || 'EUR', JSON.stringify(rest)]
                );
            }
        }
        ...
        if (Array.isArray(nextData.recurringTransactions)) {
            ...
            for (const r of nextData.recurringTransactions) {
                ...
                await client.query(
                    `INSERT INTO recurring_transactions (id, user_id, account_id, amount, frequency, start_date, description, data)
        ...
        if (Array.isArray(nextData.tasks)) {
            ...
            for (const t of nextData.tasks) {
                ...
                await client.query(
                    `INSERT INTO tasks (id, user_id, title, status, due_date, priority, data)
                     VALUES ($1, $2, $3, $4, $5, $6, $7)`,
                    [id, userId, title || 'Task', status || 'pending', dueDate || null, priority || 'medium', JSON.stringify(rest)]
                );
            }
        }
        ...
        await client.query(
            `INSERT INTO user_financial_profiles (user_id, last_updated_at, data)
             VALUES ($1, $2, $3)
             ON CONFLICT (user_id)
             DO UPDATE SET last_updated_at = EXCLUDED.last_updated_at, data = EXCLUDED.data`,
            [userId, nextUpdatedAt, JSON.stringify(profilePayload)]
        );
 ...
 */
export async function getCategoryTotalsSql(
    userId: number,
 ...
 */
export async function addOrUpdateRelationalItem(
    userId: number,
    collection: string,
 ...
 */
export async function removeRelationalItem(
    userId: number,
    collection: string,
    itemId: string
): Promise<Record<string, any>> {
    const currentData = await fetchFinancialDataFromRelational(userId);
    let items = Array.isArray(currentData[collection]) ? [...currentData[collection]] : [];
    items = items.filter((i: any) => i && i.id !== itemId);
    return syncFinancialDataToRelational(userId, { [collection]: items }, true);
}
 ...
 */
export async function applyJsonPatchToRelational(
    userId: number,
    patchOps: JsonPatchOperation[]
): Promise<{ success: boolean; doc?: Record<string, any>; error?: string }> {
    const currentData = await fetchFinancialDataFromRelational(userId);
    const patchRes = applyJsonPatch(currentData, patchOps);
