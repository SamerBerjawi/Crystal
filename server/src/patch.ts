...
function getPointerValue(doc: any, tokens: string[]): any {
    let curr = doc;
 ...
 */
export function applyOperation(targetDoc: any, op: JsonPatchOperation): any {
...
): { success: boolean; doc?: any; error?: string } {
    try {
 ...
 */
export function addOrUpdateCollectionItem(
    ...
    const items = nextDoc[collectionName] as any[];
    const itemId = item.id;
    if (!itemId) {
        items.push(item);
    } else {
        const existingIdx = items.findIndex((i: any) => i.id === itemId);
        if (existingIdx >= 0) {
...
export function removeCollectionItem(
    ...
    nextDoc[collectionName] = nextDoc[collectionName].filter((item: any) => item && item.id !== itemId);
    return nextDoc;
}
