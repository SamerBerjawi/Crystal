...
import { validateJsonPatchPayload } from './schemas';
...
try {
    const tokens = parseJsonPointer('/transactions/0/amount');
    assert(tokens.length === 3 && tokens[0] === 'transactions' && tokens[1] === '0' && tokens[2] === 'amount', 'Pointer parsing failed');
    assert(parseJsonPointer('/foo~1bar/baz~0qux')[0] === 'foo/bar', 'Escaped pointer parsing failed for ~1');
    assert(parseJsonPointer('/foo~1bar/baz~0qux')[1] === 'baz~qux', 'Escaped pointer parsing failed for ~0');
...
try {
    ...
    const patch = [{ op: 'add' as const, path: '/accounts/-', value: { id: 'acc-2', name: 'Savings' } }];
    const res = applyJsonPatch(doc, patch);
...
try {
    ...
    const patch = [
        { op: 'replace' as const, path: '/userStats/streak', value: 10 },
        { op: 'remove' as const, path: '/tasks/0' },
    ];
    const res = applyJsonPatch(doc, patch);
...
try {
    ...
    const patch = [
        { op: 'copy' as const, from: '/a/title', path: '/b/copiedTitle' },
        { op: 'move' as const, from: '/a/title', path: '/a/movedTitle' },
    ];
    const res = applyJsonPatch(doc, patch);
...
try {
    const doc = { count: 42 };
    const validTestRes = applyJsonPatch(doc, [{ op: 'test' as const, path: '/count', value: 42 }]);
    assert(validTestRes.success === true, 'Test op with matching value should succeed');

    const invalidTestRes = applyJsonPatch(doc, [{ op: 'test' as const, path: '/count', value: 99 }]);
...
try {
    let doc: Record<string, any> = { transactions: [{ id: 'tx-1', amount: 100 }] };
    doc = addOrUpdateCollectionItem(doc, 'transactions', { id: 'tx-1', amount: 150 });
    assert(doc.transactions.length === 1 && doc.transactions[0].amount === 150, 'Collection item should be updated');

    doc = addOrUpdateCollectionItem(doc, 'transactions', { id: 'tx-2', amount: 200 });
    assert(doc.transactions.length === 2 && doc.transactions[1].id === 'tx-2', 'New collection item should be added');

    doc = removeCollectionItem(doc, 'transactions', 'tx-1');
...
try {
    ...
    const validated = validateJsonPatchPayload(payload);
