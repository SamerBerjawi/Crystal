...
const SchedulePage: React.FC<ScheduleProps> = () => {
    ...
    const handleSavePostedTransaction = (transactionsToSave: (Omit<Transaction, 'id'> & { id?: string })[], idsToDelete: string[]) => {
        ...
        if (itemToPost.isRecurring) {
            ...
            // Mark the specific occurrence as skipped so it disappears from the schedule
            // The originalDateForOverride is set during the generation loop in useMemo
            const dateToSkip = itemToPost.originalDateForOverride || itemToPost.date;
            
            saveRecurringOverride({
                recurringTransactionId: rt.id,
                originalDate: dateToSkip,
                isSkipped: true
            });
        } else {
