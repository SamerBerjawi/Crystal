...
const SignIn: React.FC<SignInProps> = ({ onSignIn, onNavigateToSignUp, onEnterDemoMode, isLoading, error }) => {
  ...
  const handleBackToHome = () => {
      // Reload page to reset to initial state (which shows landing page if not authenticated)
      // Alternatively, we could pass a navigation prop, but a reload is a simple way to return to root state
  ...
  return (
    <div className="flex min-h-screen bg-light-bg dark:bg-dark-bg">
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 sm:px-12 lg:px-24 xl:px-32 relative z-10 bg-light-card dark:bg-dark-card lg:bg-transparent lg:dark:bg-transparent">
        <button onClick={handleBackToHome} className="absolute top-8 left-8 text-light-text-secondary dark:text-dark-text-secondary hover:text-primary-500 transition-colors flex items-center gap-1">
