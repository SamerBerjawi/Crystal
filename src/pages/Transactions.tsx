...
import { Transaction, Account, DisplayTransaction, RecurringTransaction, Category, Tag } from '../types';
...
const TransactionPulse = React.memo(function TransactionPulse({ transactions }: { transactions: DisplayTransaction[] }) {
    const data = useMemo(() => {
        const map = new Map<string, { date: string, income: number, expense: number }>();
        
        if (transactions.length === 0) return [];

        // Find date range to fill gaps if needed, or just map existing
        // For simplicity and "Pulse" effect, just mapping existing dates is usually fine, 
        // but sorting is crucial.
        ...
        transactions.forEach(tx => {
            ...
            if (amount > 0) {
                ...
                // Store expense as negative for chart positioning, or positive?
                // Recharts bar chart with stackId needs distinct values. 
                // Let's store expense as negative to make them go down.
    ...
    const CustomTooltip = ({ active, payload, label }: any) => {
        if (active && payload && payload.length) {
            ...
            return (
                <div className="bg-light-card dark:bg-dark-card p-3 rounded-lg shadow-lg border border-black/5 dark:border-white/5 text-xs">
                    <p className="font-bold mb-2">{date.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}</p>
                    {payload.map((p: any) => (
                        <div key={p.name} className="flex justify-between gap-4 mb-1">
                            <span className={p.name === 'Income' ? 'text-green-600' : 'text-red-600'}>{p.name}:</span>
                            <span className="font-mono font-medium">
                                {formatCurrency(Math.abs(p.value), 'EUR')}
                            </span>
                        </div>
                    ))}
                </div>
    ...
    return (
        <Card className="p-0 border border-black/5 dark:border-white/5 shadow-sm overflow-hidden h-40">
            <div className="absolute top-3 left-4 z-10 flex items-center gap-2 pointer-events-none">
                 <span className="material-symbols-outlined text-light-text-secondary dark:text-dark-text-secondary text-lg">ecg_heart</span>
                 <h3 className="text-xs font-bold uppercase tracking-wider text-light-text-secondary dark:text-dark-text-secondary">Transaction Pulse</h3>
            </div>
            <div className="w-full h-full pt-8 pb-2 px-2">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data} barGap={0} barCategoryGap="10%">
                        ...
                        <Bar dataKey="expense" name="Expense" fill="#EF4444" radius={[0, 0, 2, 2]} maxBarSize={40} />
                    </BarChart>
                </ResponsiveContainer>
            </div>
        </Card>
...
const Transactions: React.FC<TransactionsProps> = ({ initialAccountFilter, initialTagFilter, onClearInitialFilters }) => {
  ...
  const handleCloseModal = () => {
    ...
    const handleSplitTransaction = (tx: DisplayTransaction) => {
        const originalTx = transactions.find(t => t.id === (tx.isTransfer ? tx.originalId : tx.id));
        if (!originalTx) return;
        setTransactionToSplit(originalTx);
        setIsSplitModalOpen(true);
    };

    const handleSaveSplit = (updatedTx: Transaction) => {
  ...
  return (
    <div className="space-y-6 flex flex-col h-full animate-fade-in-up">
      ...
      />
       {contextMenu && (
            ...
            >
                ...
                <button onClick={() => { handleSplitTransaction(contextMenu.transaction); setContextMenu(null); }} className="w-full text-left flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-black/5 dark:hover:bg-white/10 transition-colors">
                    <span className="material-symbols-outlined text-lg text-orange-500">call_split</span>
      ...
      {/* Transaction Pulse Chart */}
      <TransactionPulse transactions={filteredTransactions} />
                        ...
                        <div className="flex-1 grid grid-cols-12 gap-3 items-center ml-3 min-w-0">
                          ...
                          {/* Column 1: Description (Expanded) */}
                            <div className="col-span-5 flex items-center gap-3 min-w-0">
                              <div
                                ...
                                style={showMerchantLogo ? undefined : { backgroundColor: isSplit ? '#F97316' : categoryColor }}
                              >
                                {showMerchantLogo && merchantLogoUrl ? (
                                  ...
                                  <span className="material-symbols-outlined text-[20px]">{isSplit ? 'call_split' : categoryIcon}</span>
                                )}
                              </div>
                          ...
                          <div className="col-span-2">
                             <span 
                                ...
                                style={isSplit ? { backgroundColor: '#FFEDD5', color: '#C2410C' } : { backgroundColor: `${categoryColor}15`, color: categoryColor }}
                             >
                                 <span className="material-symbols-outlined text-[16px] shrink-0">{isSplit ? 'call_split' : categoryIcon}</span>
                                 <span className="truncate">{tx.category}</span>
                             </span>
                             {isSplit && tx.subTransactions && (
                                 <span className="ml-2 text-xs text-light-text-secondary dark:text-dark-text-secondary">
                                     {tx.subTransactions.length} splits
                                 </span>
                             )}
                          </div>
