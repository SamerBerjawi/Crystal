...
const CashflowSankey: React.FC<CashflowSankeyProps> = ({ transactions, incomeCategories, expenseCategories }) => {
      ...
      return (
        <Layer key={`node-${index}`}>
          ...
          <text
            ...
            alignmentBaseline="middle"
            fontSize={9}
            className="font-mono fill-black dark:fill-white privacy-blur"
          >
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
        ...
        const nodeName = data.payload.name;
        const value = data.value;

        return (
            <div className="bg-light-card dark:bg-dark-card p-2 border border-black/10 dark:border-white/10 rounded shadow-sm text-xs">
                {sourceName && targetName ? (
                     <p><span className="font-semibold">{sourceName} → {targetName}</span>: <span className="privacy-blur">{formatCurrency(value, 'EUR')}</span></p>
                ) : (
                     <p><span className="font-semibold">{nodeName}</span>: <span className="privacy-blur">{formatCurrency(value, 'EUR')}</span></p>
                )}
            </div>
        );
    }
