...
const SCHEMA_CONFIG: SchemaConfig = {
    transactions: {
        fields: [
            ...
            { key: 'description', label: 'Description', required: true, keywords: ['description', 'payee', 'merchant', 'details', 'narrative', 'memo'] },
    ...
    budgets: {
        fields: [
            { key: 'categoryName', label: 'Category', required: true, keywords: ['category', 'name'] },
            { key: 'amount', label: 'Amount', required: true, keywords: ['amount', 'limit', 'budget'] }
        ]
    },
    schedule: {
        fields: [
             { key: 'description', label: 'Description', required: true, keywords: ['description', 'name'] },
             { key: 'amount', label: 'Amount', required: true, keywords: ['amount'] },
             { key: 'frequency', label: 'Frequency', required: true, keywords: ['frequency', 'interval', 'repeat'] },
             { key: 'nextDueDate', label: 'Next Due Date', required: true, keywords: ['date', 'next due', 'start date'] },
             { key: 'type', label: 'Type', required: false, keywords: ['type'] },
             { key: 'account', label: 'Account', required: false, keywords: ['account'] }
        ],
        hasAccountSource: true
    },
    investments: {
         fields: [
             { key: 'symbol', label: 'Symbol', required: true, keywords: ['symbol', 'ticker'] },
             { key: 'name', label: 'Name', required: false, keywords: ['name'] },
             { key: 'quantity', label: 'Quantity', required: true, keywords: ['quantity', 'shares', 'units'] },
             { key: 'price', label: 'Price', required: true, keywords: ['price', 'cost', 'value'] },
             { key: 'date', label: 'Date', required: true, keywords: ['date'] },
             { key: 'type', label: 'Type (Buy/Sell)', required: true, keywords: ['type', 'action'] },
             { key: 'account', label: 'Account', required: false, keywords: ['account'] }
         ],
         hasAccountSource: true
    },
...
const ImportWizard: React.FC<ImportWizardProps> = ({ importType, onClose, onPublish, existingAccounts, allCategories }) => {
    ...
    const processConfiguredData = () => {
        ...
        parsedData.forEach((row, index) => {
            ...
            fields.forEach(field => {
                ...
                // Date Parsing
                if ((field.key.toLowerCase().includes('date') || field.key === 'nextDueDate' || field.key === 'expiryDate') && value) {
                ...
                // Number Parsing
                if ((field.key === 'amount' || field.key === 'balance' || field.key === 'total' || field.key === 'quantity' || field.key === 'price' || field.key === 'currentAmount') && value !== undefined) {
            ...
            // Common logic for types that have account source
            if (schema.hasAccountSource) {
            ...
            // Default currency
            if (!newRow.currency && importType !== 'tags' && importType !== 'categories') {
                 newRow.currency = 'EUR';
            }
    ...
    const handleProcessCleaning = () => {
        ...
        // Map Accounts (Transactions, Investments, Schedule)
        if (schema.hasAccountSource && accountSource === 'column') {
    ...
    const handlePublish = () => {
        ...
        // Create new accounts if needed (only for types that use account mapping)
        if (schema.hasAccountSource && accountSource === 'column') {
            Object.entries(accountMap).forEach(([csvName, mappedValue]) => {
                ...
                if (val.startsWith('_CREATE_NEW_:')) {
                    ...
                    newAccountsToCreate.push({
                        id: newId,
                        name: newAccountName,
                        type: 'Checking', // Default type for auto-created
                        balance: 0, 
        ...
        const dataToPublish = cleanedData
          ...
          .map(row => {
              ...
              if (schema.hasAccountSource) {
                  let accountId;
                  if (accountSource === 'single') accountId = selectedSingleAccountId;
                  else {
                  ...
                  row.accountId = accountId; // Standardize to accountId
              }
              ...
              if (importType === 'transactions') {
                  ...
                  return { ...row, category: finalCat, type: row.amount >= 0 ? 'income' : 'expense' };
              }
              ...
              // Clean up properties
              const { originalIndex, ...finalRow } = row;
              return finalRow;
          }).filter(Boolean);
        ...
        <div className="max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-light-card dark:bg-dark-card p-6 rounded-lg shadow-sm">
                ...
                {fields.map((field: any) => {
                    if (field.key === 'account' && schema.hasAccountSource && accountSource === 'single') return null; 
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6 bg-light-card dark:bg-dark-card p-6 rounded-lg shadow-sm">
                ...
                {(importType === 'transactions' || importType === 'investments' || importType === 'schedule') && (
                    <div>
...
const Step5Map: React.FC<any> = ({ importType, categories, setCategoryMap, categoryMap, allCategories, accounts, setAccountMap, accountMap, existingAccounts, accountTypes, setAccountTypeMap, accountTypeMap, schema }) => (
    <div className="max-w-3xl mx-auto space-y-8">
        ...
        {/* Account Mapping */}
        {schema?.hasAccountSource && accounts.length > 0 && (
             <div className="bg-light-card dark:bg-dark-card p-6 rounded-lg">
